...
ggplot(data=reads_transcript, 
  ...
  geom_point() +
  theme(legend.position="none") +
  labs(x = "Reads", y = "Percentage of transcripts")
...
ggplot(data=reads_gene, 
       aes(num_read, unique_genes, group=sample, colour=sample)) +
  geom_line() +
  geom_point() +
  geom_point(data=subset(reads_gene, i %in% c(n_freq)), aes(num_read, unique_genes), shape = 21, fill = 'white', size=2) +
  theme(legend.position="none") +
